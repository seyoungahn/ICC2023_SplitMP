...
class MobilityDataGenerator(object):
    def __init__(self, syst_conf):
        self.n_nodes = syst_conf['n_nodes']
        self.syst_dim = (syst_conf['min_dim'], syst_conf['max_dim'])
        self.syst_vel = (syst_conf['min_vel'], syst_conf['max_vel'])
        self.max_wait = syst_conf['waiting_time']
        self.init_stationary = syst_conf['init_stationary']

    def __iter__(self):
        ndim = len(self.syst_dim)
        min_vel, max_vel = self.syst_vel
        ...
        while True:
            ...
            arrived = np.where(np.logical_and(d <= velocity, wait_time <= 0.))[0]
            ...
            if self.max_wait:
                ...
                wait_time[np.where(velocity == 0.)[0]] -= 1.
                # update info for moving nodes
                arrived = np.where(np.logical_and(velocity == 0., wait_time < 0.))[0]
    ...
    class System(object):
        def __init__(self, syst_conf):
            ...
            self.nodes = []
            for i in range(syst_conf['n_nodes']):
                self.nodes.append(Node(i, syst_conf))